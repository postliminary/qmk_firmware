#include "hold_to_reset.h"

static uint16_t reset_keycode;  // Which key starts reset hold
static uint32_t key_timer;              // Hold long has the key been pressed

void hold_to_reset_init(uint16_t hold_to_reset_keycode) {
    reset_keycode = hold_to_reset_keycode;
}

bool process_hold_to_reset(uint16_t keycode, keyrecord_t* record) {
    switch (keycode) {
        case reset_keycode:
            if (record->event.pressed) {
                key_timer = timer_read32();
            } else {
                if (timer_elapsed32(key_timer) >= HOLD_TO_RESET_DELAY) {
                    reset_keyboard();
                }
            }
            return true;
        default:
            return false; //Process all other keycodes normally
    }
}
